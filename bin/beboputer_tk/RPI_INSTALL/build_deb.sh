#!/usr/bin/env bash
# ================================================================
# build_deb.sh — Build beboputer_<version>_all.deb (tkinter build)
#
# Mirrors bin/beboputer_v7/RPI_INSTALL/build_deb.sh exactly, launching
...
# -- this is meant as the eventual successor build, not a separate
# product, so installing it is a normal upgrade over an existing
# Qt-build install of the same package name.
#
...
# (__version__) — beboputer_tk re-exports the exact same constant, so
# this always tracks the app regardless of which UI build is packaged.
...
echo "============================================================"
echo " Beboputer Raspberry Pi .deb Builder (tkinter build)"
...
#!/usr/bin/env bash
exec python3 /usr/share/beboputer/bin/run_beboputer_tk.py "$@"
EOF
...
Description: PY-DIYCALCULATOR — Beboputer Virtual 8-bit Computer
 A tkinter desktop application that emulates a virtual 8-bit CPU
