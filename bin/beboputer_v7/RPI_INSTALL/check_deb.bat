...
wsl bash -c "DEB=$(ls -t '%WSL_DIST%'/beboputer_*_all.deb 2>/dev/null | head -1); if [ -z \"$DEB\" ]; then echo 'No beboputer_*_all.deb found in dist/. Run build_deb.sh first.'; exit 1; fi; echo \"Inspecting: $DEB\"; dpkg-deb -c \"$DEB\" | grep -E '(BITMAPS|Config|Data|Data Book|WorkInProgress|tutorial)' | head -20" > "%OUT%" 2>&1
