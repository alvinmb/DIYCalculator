...
MAINLOOP:  JSR     [GETNUM]   # Get 1st 16-bit number (NUMA)
           ...
           JSR     [_ADD]     # Add NUMA and NUMB together
...
_GN_RADD:  .2BYTE             # 2-byte temp location
...
_DN_RET:   LDA     [_DN_RADD+1] # Get LS byte of return address
           PUSHA              # temp location and push onto stack
           LDA     [_DN_RADD] # Get MS byte of return address
           PUSHA              # temp location and push onto stack
           RTS                # Return from this subroutine

_DN_RADD:  .2BYTE             # 2-byte temp location
...
#######################################################################
# _ADD subroutine -- add two 16-bit unsigned numbers                  #
#######################################################################
...
_AD_GNUM:  POPA               # Retrieve MS byte of addend
           STA     [_AD_NUMB] # stack and store it
           POPA               # Retrieve LS byte of addend
           STA     [_AD_NUMB+1] # stack and store it
           POPA               # Retrieve MS byte of augend
           STA     [_AD_NUMA] # stack and store it
           POPA               # Retrieve LS byte of augend
...
_AD_DOIT:  ADD     [_AD_NUMB+1] # Add LS byte of addend to ACC
           ...
           ADDC    [_AD_NUMB] # Add MS byte of addend with carry
           PUSHA              # and push MS sum onto stack

_AD_CHK:   JNC     [_AD_RET]  # If carry flag = 0 jump to return
           BLDX    MSG_001    # Load X reg with addr of message
...
_AD_RADD:  .2BYTE             # Return address
_AD_NUMA:  .BYTE              # MS byte of augend
_AD_NUMB:  .2BYTE             # Addend
########## End of _ADD subroutine
