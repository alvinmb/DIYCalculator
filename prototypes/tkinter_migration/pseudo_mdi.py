"""
pseudo_mdi.py -- a QMdiArea/QMdiSubWindow-alike for plain tkinter.

tkinter has no built-in MDI container. This gives you the same shape:
    ...
    MdiChild  -- a floating sub-window inside that Frame: title bar
                 ...
                 as QMdiSubWindow (minus minimize, which the real app
                 doesn't use either).

Both are built entirely from tk.Frame/Label/Canvas + place() geometry
management and mouse-event bindings -- no third-party packages.

Layout helper
-------------
tile_children(area, children, margin=8) arranges an MdiArea's children
without overlap, using the same 3-tier fallback idea as this project's
_layout_startup_panels() in main_window.py: side-by-side if everything
fits, else a 2-row wrap, else a stacked column -- just computed against
the MdiArea's current size instead of the screen's.

Run this file directly for a live demo:

    python pseudo_mdi.py
"""
...
def _demo():
    ...
    specs = [
        PanelSpec("Calculator", 340, 420),
        PanelSpec("Memory Walker", 520, 380),
        PanelSpec("Message Display", 300, 160),
    ]
    children = tile_children(area, specs)

    calc, walker, msg = children

    # populate Calculator with a small button grid, like DIYButton would
    grid = tk.Frame(calc.content, bg="#D4D0C8")
    grid.pack(padx=8, pady=8)
    for r in range(4):
        for c in range(4):
            tk.Button(grid, text=str(r * 4 + c), width=4, height=2).grid(
                row=r, column=c, padx=2, pady=2)

    tree = ttk.Treeview(walker.content, columns=("addr", "data"), show="headings")
    tree.heading("addr", text="Address")
    tree.heading("data", text="Data")
    for i in range(12):
        tree.insert("", "end", values=(f"${0x4000+i:04X}", f"${i:02X}"))
    tree.pack(fill="both", expand=True, padx=6, pady=6)

    tk.Label(msg.content, text="Ready.", bg="#D4D0C8", anchor="w",
              font=("Consolas", 10)).pack(fill="both", expand=True, padx=6, pady=6)

    def add_window():
        n = len(area.children_windows) + 1
        w = area.add_child(f"Extra {n}", x=40 + 15 * n, y=40 + 15 * n,
                            width=260, height=180)
        tk.Label(w.content, text=f"Panel #{n}", bg="#D4D0C8").pack(expand=True)

    view_menu.add_command(label="New floating panel", command=add_window)
